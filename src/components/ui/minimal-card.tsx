...
/*
 * Cult UI ships this with hardcoded neutral-* colors and pure-black shadow
 * layers — replaced with ATP's own surface tokens (already theme-aware, so
 * no dark: variant needed) and the warm near-black used by tokens.css'
 * own shadow scale instead of rgba(0,0,0,...).
 */
...
>(({ className, children, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      'bg-surface-alt hover:bg-surface rounded-[24px] p-2 no-underline shadow-sm transition-colors',
      'shadow-[0px_1px_1px_0px_rgba(28,25,38,0.05),0px_1px_1px_0px_rgba(255,252,240,0.5)_inset,0px_0px_0px_1px_hsla(0,0%,100%,0.1)_inset,0px_0px_1px_0px_rgba(28,25,38,0.5)]',
...
>(({ className, ...props }, ref) => (
  <p ref={ref} className={cn('text-text-secondary px-1 pb-2 text-sm', className)} {...props} />
));
