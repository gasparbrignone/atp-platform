...
const innerSpanVariants = cva(
  ...
  ],
  {
    variants: {
      ...
      /*
       * Cult UI ships several named decorative presets (ocean/candy/forest/
       * sunset/nebula) with raw hex stops unrelated to ATP — kept as-is,
       * they're intentionally arbitrary decorative options, not brand
       * colors. `atp` is added on top using the real brand tokens
       * (--color-primary, --color-secondary) and is the one this project
       * should actually use for on-brand CTAs.
       */
      gradient: {
        atp: 'bg-[conic-gradient(from_90deg_at_50%_50%,var(--color-primary)_0%,var(--color-secondary)_50%,var(--color-primary)_100%)]',
...
export interface UnifiedButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  gradient?: 'atp' | 'sunrise' | 'ocean' | 'candy' | 'default' | 'forest' | 'sunset' | 'nebula';
}

const BgAnimateButton = React.forwardRef<HTMLButtonElement, UnifiedButtonProps>(
  (
    {
      // `variant`/`showBackground` don't drive any style here (only `gradient`
      // does) — destructured only to strip them out of `...props` so they
      // don't leak onto the DOM element as invalid attributes.
    ...
    return (
      <Comp className={cn(outerDivVariants({ size, rounded }), className)} ref={ref} {...props}>
        {gradient && <span className={cn(innerSpanVariants({ gradient, animation }))} />}
